...
export const Login = (props) => {
  ...
  async function PostLogin(e) {
    ...
    pathname =
      pathname === '/mifugo/login' ? '/auth/log-in/client' : '/auth/log-in/vet';
    ...
    if (validation_info === 'valid') {
      ...
      if (response.vp_P_A === pass.vet) {
        ...
        props.history.push('/mifugo/vet/homepage');
      } else if (response.vp_P_A === pass.client) {
        utils.clearContextErrors(dispatch);
        props.history.push('/mifugo/client/homepage');
      } else if (response.vp_P_A === pass.vetUpdate) {
        utils.clearContextErrors(dispatch);
        props.history.push('/mifugo/vet/completeRegistration');
      }
    } else {
